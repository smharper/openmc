...
! The optional allow_rand argument can be set to .false. to prevent the function
! from adjusting the coordinates using the random translation table.  This
! optional argument is only in place to allow easy lattice cell edge finding
! e.g. in distance_to_boundary.

    function get_local_xyz_(this, global_xyz, i_xyz, allow_rand) &
         &result(local_xyz)
      import Lattice
      class(Lattice),    intent(in) :: this
      real(8),           intent(in) :: global_xyz(3)
      integer,           intent(in) :: i_xyz(3)
      real(8)                       :: local_xyz(3)
      logical, optional, intent(in) :: allow_rand
...
!===============================================================================
  ...
  function get_local_rect(this, global_xyz, i_xyz, allow_rand) result(local_xyz)
    class(RectLattice), intent(in) :: this
    real(8),            intent(in) :: global_xyz(3)
    integer,            intent(in) :: i_xyz(3)
    logical, optional,  intent(in) :: allow_rand
    ...
    real(8) :: xyz(3)      ! global_xyz alias
    logical :: allow_rand_ ! accounts for allow_rand presence and value
...
!===============================================================================
  ...
  function get_local_hex(this, global_xyz, i_xyz, allow_rand) result(local_xyz)
    class(HexLattice), intent(in) :: this
    real(8),           intent(in) :: global_xyz(3)
    integer,           intent(in) :: i_xyz(3)
    logical, optional, intent(in) :: allow_rand
